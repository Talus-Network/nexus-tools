...
#[test]
fn math_binary_accepts_current_workbench_toolkit_config() {
    let temp_dir = TempDir::create();
    let config_path = temp_dir.0.join("toolkit-config.json");
    fs::write(&config_path, r#"{"signed_http":{"mode":"disabled"}}"#)
        .expect("current Workbench Toolkit config should be written");
    ...
    loop {
            ...
            .is_some()
        {
            ...
            panic!(
                "{}",
                process_failure(
                    "math binary exited before accepting the current Workbench Toolkit config",
                    output,
                )
            );
        }
        ...
        if Instant::now() >= deadline {
            ...
            panic!(
                "{}",
                process_failure(
                    "math binary did not listen after accepting the current Workbench Toolkit config",
                    output,
                )
            );
        }
